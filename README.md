...
This repo now includes a Docker setup for Python 3.6 + TensorFlow 1.12 + MPI.
...
### 2) Run normal training
```bash
docker compose run --rm train
```

### 3) Run MPI training
Default process count is 4. You can change it via `MPI_NP`.
```bash
docker compose run --rm -e MPI_NP=4 train-mpi
```

### 4) Run other scripts
```bash
docker compose run --rm train python human_play.py
```

### GUI troubleshooting (Windows + VcXsrv)
If `human_play.py` seems to run but you don't see any window:
```bash
docker compose run --rm -e DISPLAY=host.docker.internal:0.0 train python gui_smoke_test.py
```
If this test window appears, your X server path is correct and `human_play.py` should also show a window.

Quick start on Windows after installing VcXsrv:
```bat
launch_gomoku.bat
```

Note:
* `human_play.py` uses `pygame` GUI. If you run inside Docker Desktop without X/GUI forwarding, use training scripts first.
* Models and outputs are persisted because the project folder is mounted into the container (`.:/workspace`).
